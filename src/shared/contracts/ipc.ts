...
export const IPC = {
  ...
  captureStart: 'capture:start', captureGetContext: 'capture:get-context', captureReadyToShow: 'capture:ready-to-show', captureArmVideoFrame: 'capture:arm-video-frame', captureProvideVideoFrame: 'capture:provide-video-frame', captureCancelVideoFrame: 'capture:cancel-video-frame', captureFreeze: 'capture:freeze', captureAnalyze: 'capture:analyze', captureAnalysisProgress: 'capture:analysis-progress', captureCancelAnalysis: 'capture:cancel-analysis', captureGetOcrLanguages: 'capture:get-ocr-languages', captureSetOcrLanguage: 'capture:set-ocr-language', captureSelect: 'capture:select', captureCancel: 'capture:cancel',
...
export const SETTINGS_CATEGORIES = ['Account', 'Models', 'Prompts', 'Recipes', 'Capture', 'Appearance', 'History', 'Privacy', 'Updates', 'About'] as const
export type SettingsCategory = (typeof SETTINGS_CATEGORIES)[number]
export function isSettingsCategory(value: unknown): value is SettingsCategory {
  return typeof value === 'string' && SETTINGS_CATEGORIES.some((category) => category === value)
}
...
/** `supported` is false where the platform or launch cannot use live selection at all. */
export interface LiveCaptureState { enabled: boolean; supported: boolean }
export type OnboardingTestCaptureResult =
  | { status: 'captured'; thumbnailDataUrl: string }
  | { status: 'cancelled' }
interface CaptureContextBase { width: number; height: number; minSelectionSize: number; displayId?: string; canEditBeforeSending: boolean }
export interface LiveCaptureContext extends CaptureContextBase { surface: 'live'; imageDataUrl: null }
export interface FrozenCaptureContext extends CaptureContextBase { surface: 'frozen'; imageDataUrl: string }
export type CaptureContext = LiveCaptureContext | FrozenCaptureContext
export type CaptureFreezeReason = 'edit' | 'analyze'
export interface CaptureVideoFrameMetadata { viewport: { width: number; height: number }; rectangle?: Rectangle }
...
export interface FoveaApi {
  ...
  application: { openSettings(category?: SettingsCategory): Promise<void> }
