...
import { IPC, type FoveaApi, type QuestionViewState, type SettingsCategory, type SettingsViewState, type WindowChromeState } from '@shared/contracts/ipc'
...
import { LiveVideoFrameCapture } from './live-video-frame'
...
const liveVideoFrame = new LiveVideoFrameCapture(
  {
    arm: () => invokeResult(IPC.captureArmVideoFrame),
    provide: (png, metadata) => invokeResult(IPC.captureProvideVideoFrame, png, metadata),
    disarm: () => invokeResult(IPC.captureCancelVideoFrame)
  },
  {
...
const api: FoveaApi = {
  ...
  application: { openSettings: (category) => invokeResult(IPC.applicationOpenSettings, category) },
...
function subscribe<T extends SettingsViewState | SettingsCategory | AppearanceState | WindowChromeState | QuestionViewState>(channel: string, callback: (value: T) => void): () => void {
